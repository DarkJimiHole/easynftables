...
def describe_code(metadata: dict, code: str) -> tuple[str, str]:
    for province in metadata["provinces"]:
        if str(province["code"]) == code:
            return code, str(province["name"])
        for city in province.get("cities", []):
            if str(city["code"]) == code:
                return code, str(city["name"])
    raise SystemExit(f"Unknown region code: {code}")
...
def main() -> int:
    ...
    if args.command == "list-provinces":
        ...
        for code in args.codes:
            region_code, name = describe_code(metadata, code)
            print(f"{region_code}\t{name}")
